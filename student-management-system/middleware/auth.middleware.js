  ...
  try {
    ...
    if (scheme !== 'Bearer' || !token) {
      return res
        .status(401)
        .json({
          message: 'รูปแบบ Authorization ไม่ถูกต้อง (ต้องเป็น Bearer <token>)',
        })
    }
...
function requireRole(...roles) {
  return function (req, res, next) {
    if (!req.user || !req.user.role) {
      return res
        .status(403)
        .json({ message: 'ไม่มีสิทธิ์เข้าถึง (role ไม่ถูกต้อง)' })
    }

    if (!roles.includes(req.user.role)) {
      return res
        .status(403)
        .json({ message: 'ไม่มีสิทธิ์เข้าถึง (role ไม่เพียงพอ)' })
    }
